function [ks_spectrograms, time_axis, f_axis] = ks_get_spectrograms(VNS_dat, plot_out, varargin);
...
%[ks_test, time_axis] = tstat_time_course(VNS_dat.ecog_theta_env(is_good,:), VNS_dat.stim_onsets_inds, VNS_dat.sampFreq, window_len);
ecog_dat = VNS_dat.ecog_theta_env(is_good,:);
%ecog_dat(:,~is_good_time) = NaN;
[ks_test, time_axis] = ks_divergence_time_course(ecog_dat, VNS_dat.stim_onsets_inds, VNS_dat.sampFreq, window_len);
f_axis = mean([4,7; 8,15; 18, 30; 33 55; 70 150],2);  % mean frequencies of each band


ks_spectrograms = zeros(size(ks_test,1), size(ks_test,2), length(f_axis));
...
for b = 1:length(bands)
    ...
    [ks_test, time_axis] = kl_divergence_time_course_2(ecog_dat, VNS_dat.stim_onsets_inds, VNS_dat.sampFreq, window_len, kl_win);
    ks_spectrograms(:,:,b) = ks_test;
%[ks_test, time_axis] = tstat_time_course(VNS_dat.ecog_alpha_env(is_good,:), VNS_dat.stim_onsets_inds, VNS_dat.sampFreq, window_len);
end


cmax = max(ks_spectrograms(:));
cmin = min(ks_spectrograms(:));
if plot_out
    is_sig_ch = sum(is_sig,2)>3;
    kl_spectrograms_sig = ks_spectrograms(is_sig_ch,:,:);
    ...
    if plot_spectrograms
        ...
        if save_figs
            ...
            saveas(gcf,[fig_dir filesep 'KS_Spectrograms_' figure_appellation '.png']);
        end
    end
    if plot_time_series
        ...
        for k = 1:length(band_names)
            subplot(2,3,k)
            kl_band = squeeze(ks_spectrograms(is_sig(:,k),:,k));
            plot(repmat(time_axis,size(kl_band,1),1)', kl_band')
            hold on; plot([0 0], get(gca, 'YLim'),'k'); plot([0 0], get(gca, 'YLim'),'k')
            xlabel('Time (s)'); ylabel('KS-Statistic'); legend('Ch 1','Ch 2','...')
            %title(['Time Series of KS-Divergence of ' band_names{k} ' Envelope'])
        ...
        if save_figs
            ...
            pause(5)
            set(gcf, 'PaperPositionMode', 'auto')
            saveas(gcf,[fig_dir filesep 'KS_Divergence_TimeCourse_' figure_appellation '.png']);
        end
    end
a=1;
